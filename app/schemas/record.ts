...
/**
 * 觀影紀錄表單的驗證規則。`/app/records/new` 與 `/edit` 共用。
 *
 * ── 必填只有三個 ──────────────────────────────────────────────
 * 片名、日期、場所。這是 SPEC 的核心設計（US-4：散場走出影廳的路上
 * 三十秒內記完），其餘一律選填——「懶得填的時候不會被擋住」（US-8）。
 * 加必填等於破壞這個產品的主張，改動前請先看 SPEC。
 *
 * ── 票價的空值語意 ────────────────────────────────────────────
 * `cost` 是 `number | null`，**null 與 0 是兩件事**：
 *   null → 沒有票價資料（編輯時代表「刪掉那筆 viewing_record_cost 列」）
 *   0    → 真的沒花錢（招待票、影展贈票）
 * 匯入管線在這件事上已經踩過坑——把「沒資料」寫成 0 會稀釋平均票價。
 * 所以這裡刻意不用 `.default(0)`，也不要把空字串 coerce 成 0。
 */
