...
/**
 * 侵權通知表單（`SCREENS §15.3`、著作權法 §90-6）。⚠️ 這份 schema 是伺服器那一份的**鏡像**，
 * 界限（100／200／50／2000／500、`min(10)`、善意聲明必為 true）逐項對齊。前端這份的作用不是
 * 把關（把關在伺服器）而是**讓人在送出之前就知道哪裡不對**。
 */
/*
 * 這個窗口的使用者是法務或權利人，操作失敗率比一般使用者高，而窗口沒開就不符 §90-4 第 3 款
 * ⇒ **擋掉他們的代價是法遵要件失效**，每一條訊息都要說「怎麼修」不是「你錯了」。
 * ⚠️ 兩份分岔的症狀最難查：前端過了、伺服器回 422，而 422 的訊息長在另一個檔案裡。
 */
...
export const takedownSchema = z.object({
  ...
  targetUrl: z
    ...
    // ⚠️ 訊息裡**刻意不寫任何網域**：這份 schema 是純模組（伺服器那份也 import 它）讀不到
    //    `siteUrl`，而本站的網域會變。寫死的代價不是不精確，是**在受理窗口上叫權利人去貼一個
    //    不是本站的位址**。真正的來源示範由輸入框的 placeholder 提供，那裡讀得到 siteUrl。
    .pipe(z.url('請貼上完整的網址，要包含開頭的 https://').max(500, '最多 500 字')),

  // §90-6 的要件：沒有勾就不是一份完整的通知。
  // ⚠️ 伺服器那份是 `z.literal(true)`，這裡是 `boolean` + `refine`——驗證行為一模一樣，
  //    差別純粹在型別：`literal(true)` 會讓「還沒勾」這個合法的中間狀態在型別上不存在，
  //    而 `UForm` 的 `:state` 要吃得下它。真正的把關在伺服器。
 ...
 * ——伺服器的 `resolveTargetFilm()` 就是這樣做的。畫面上回顯的東西必須跟資料庫裡
 * `target_film_id` 真正會填進去的**是同一個判斷**：回顯認得出、伺服器認不出，使用者會以為
