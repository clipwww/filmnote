...
/**
 * US-47 帳號刪除。**隱私權政策第 5 節寫著「你隨時可以刪除帳號與所有資料」**——在這一區出現
 * 之前，那是一句我們聲明了但使用者做不到的話。這裡只做三件事：先給預覽、要求打字確認、
 * 把結果如實講完（資料層在 `0009`，端點在 `server/api/account/delete.post.ts`）。
 */
/*
 * 一定要先預覽，因為「會刪掉什麼」在這個資料模型裡**不是顯而易見的**：你自建的作品只要已
 * 核准、被別人的紀錄用到、或已合併，就**不會**跟著消失。`account_deletion_preview()` 與端點
 * 清 bucket 用的是**同一份判準**（`account_purgeable_films`），不是兩份會漂移的條件。
 */
/*
 * 打字確認而不是「你確定嗎？」：端點的 `confirm` 要等於自己的 username，比對不過回 422 且
 * **一列都不動**。它比緩衝期好的地方是同步的——不必依賴一個「三十天後真的會有東西去執行」
 * 的排程。
 */
/*
 * ⚠️ 成功之後**不要自動跳走**，但也**不要讓人繼續用這一頁**：cookie 已被清掉、profile 也沒了，
 * 再發任何查詢都會回 401，那看起來像壞掉不像刪成功。做法是把結果留在同一個對話框裡讓人讀完
 * （那是他最後一次看到自己資料的機會），由他自己按鈕離開。對話框本身不查任何東西。
 */
  ...
  <section>
    ...
    <template v-else>
      ...
      <dl class="mt-3 border border-default rounded-sm text-sm divide-y divide-default">
        ...
        <!--
          ⚠️ 這一列的理由**不只一個**，措辭不可以只寫其中一個：`account_purgeable_films` 排除四種
             情況（已核准、被別人的紀錄引用、被合併、出現在合併紀錄裡）。實測 David 的 16 部全部
             落在「已被合併」那一條，如果這裡只寫「被別人引用」，那對他就是一句錯的話。
        -->
