...
/**
 * 金額摘要那一句。**只在 client 端執行**（呼叫端以 `<ClientOnly>` 包住）。
 *
 * 資料與「每年花費」那條 band 共用 `useUserSpend()`——**同一個 key、同一次請求、
 * 同一份答案**。兩份查詢一定會在某次修改後對「看不看得到」「是不是全部」
 * 給出不同答案，而沒有人會發現，因為沒有人會把同一頁的兩個地方擺在一起看。
 *
...
<template>
  <!--
    ★ `canSeeMoney` 為 false ⇒ **整個章節不存在**（`SCREENS §12-3`）。
      不是畫成 0、不是打馬賽克——否則 `總花費 ÷ 場次` 就能反推個別票價，
      隱私是假的。也不留佔位：`§12-4`「絕不渲染 NT$ ———」，那等於公告
