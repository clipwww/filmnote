 ...
 * 法律文件正文（`SCREENS §15.1`）。
 *
 * ── 為什麼不用 Nuxt UI 的 prose 樣式 ──────────────────────────────────
 * `SCREENS §15.1` 明文禁止：那一套是為技術文件調的，字級與行高跟本站的紙
 * 不同調，而且會把 `<code>` 渲染成等寬字（DS §0 排除等寬字當資料微標籤）。
 *
 * ── 行長：`34em` 而不是 `34ch` ────────────────────────────────────────
 * DS §2.4 寫的是「中文內文 `max-width: 34ch`（≈560px）」，兩個數字對不起來，
 * 而對得起來的是後面那一個。`ch` 是**當前字型「0」的推進寬度**，本站的
 * 字型堆疊裡供應數字的是 Inter（`0` ≈ 0.6em）或退到蘋方（半形，0.5em），
 * 也就是 `34ch` 實測落在 272–330px、一行 17–20 個漢字，不是規格說的 ≈560px。
 * 漢字是 1 em 全形，所以「34 個字」寫成 CSS 就是 `34em`＝544px——
 * 這才是 DS 那一行想表達的東西（§2.4 自己也是用 em 推導的：31.5 em ≈ 32 個漢字）。
 * ⚠️ 這是**對帳結果不是自由發揮**：要改回 `ch` 之前先量一次一行有幾個字。
 *
 * ── 只有文字區塊受行長限制 ────────────────────────────────────────────
 * 表格不受：一個被壓在 544px 裡的三欄表會逐格換行，讀不出對照關係
 * （視覺稿的 copyright 那一段就是 `max-width:none`）。表格改為在自己的
 * `overflow-x-auto` 容器內捲動——DS §10：頁面 body 永遠不橫向捲動。
 */
