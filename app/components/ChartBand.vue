...
/**
 * `/app` 的一條 band（`SCREENS.md §9`）。垂直長卷、一個 band 一張圖、滿容器寬，
 * 不是 2×2 的卡片牆。
 *
 * ⚠️ **圖表一律畫在這張卡片上（`bg-default` 票根紙），不要直接放在裸台紙上。**
 * 零格對背景的對比在兩種紙上不一樣：亮色放卡片上 1.27:1、放台紙上只剩
 * 1.16:1，零格會跟頁面底糊在一起，整張圖看起來「少了一層」（§1.3）。
 * 這是台紙變暖之後才出現的新限制。
 *
 * 每個 band 都要有資料表 fallback（§5.6）：ECharts 的 heatmap 不支援 decal，
 * 色盲友善完全靠單色相明度階，而螢幕閱讀器從 canvas 什麼都拿不到。
 * 這份表同時也是「我就是想看數字」的人要的東西，以及對帳破洞的工具
 * （§5.4b 三之 1：資料表有值的格子在圖上都要有顏色）。
 */
defineProps<{
  title: string
