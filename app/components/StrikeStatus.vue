 ...
 *
 * ── 為什麼一定要有這一區 ─────────────────────────────────────────────
 * `/legal/terms` 第 6 節寫著「三次涉有侵權情事應終止全部或部分服務」。
 * 條款寫了但使用者查不到自己有幾次，**第三次就是突襲**。§90-4 要求的是
 * 「確實履行」，而一個查不到的計數在使用者那一側等於不存在。
 *
 * ── `strike_count = 0` 時整個區塊不出現 ──────────────────────────────
 * 沒有事的人不需要被提醒有三振制度。這不是把資訊藏起來——制度本身寫在條款裡，
 * 這一區是「你自己的狀態」，而沒有狀態就不該有版面。
 *
 * ── ⚠️ 計數只能來自資料庫，不可以自己算也不可以快取 ──────────────────
 * `admin_restore()` 會把該次通知造成的三振 `revoked_at` 掉，`strike_count`
 * 由 `apply_three_strikes()` trigger 重算。使用者在意的正是「我申訴成功了沒」，
 * 所以這裡 `getCachedData` 直接回 undefined：每次進到這一頁都重新問一次。
 * 拿列表 `length` 當計數也不行——那是第二份判斷，會跟 trigger 分岔。
 */
