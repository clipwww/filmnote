...
/**
 * 多刷排行（`SCREENS.md §9` band 7、US-41）。⚠️ 「多刷」的語意**跟著檢視視角變**：指定年份
 * 是「那一年內看過兩次以上」、全期是「這些年來看過兩次以上」⇒ 呼叫端的圖說要講清楚範圍，
 * 抽屜的標題與過濾也要跟著同一個 scope（`SCREENS §9c.2`）。
 */
defineProps<{ items: YearStats['repeats'] }>()

/**
 * 整列可點 → 抽屜列出那一部片在目前 scope 內的每一次觀看紀錄。★ 只 emit `film_id`，
 * **不 emit slug 或片名當識別**（多刷以 film_id 分組，片名會撞、slug 對未審核 UGC 是 404）。
 * ★ `records` 一起帶出去給標題用——那是排行上的數字，跟抽屜列幾張擺一起才看得出不一致（#169）。
 */
...
<template>
  <ul class="space-y-3">
    <li v-for="f in items" :key="f.film_id">
      <!--
        ★ **整列是一顆 button，片名不再是 NuxtLink**：`<a>` 巢在 `<button>` 裡是無效 HTML，而且點到
          片名時連結會吃掉事件、抽屜開不起來——使用者只會覺得「有時候點得開有時候點不開」。
      -->
      <!--
        作品頁的入口在抽屜裡：每張 TicketCard 的片名本來就連到 `/film/{slug}`，而且那個 slug 來自
        **紀錄**、已經過了 public + approved 的閘門——排行的 `repeats[].slug` 直接來自 left join，
        **沒有過閘門**。★ 不要加 hover 位移／transition；focus 樣式照 `YearStrip` 的 outline 寫法。
      -->
      <button
        type="button"
