 ...
 *
 * ── 為什麼一定要在瀏覽器縮 ───────────────────────────────────
 * `ugc-poster` bucket 的 `file_size_limit` 是 **2 MiB**，而現在手機隨手拍的
 * 照片就 3–5 MB。不縮的話使用者會在按下上傳之後才被拒絕，而那時他已經
 * 完成了整個表單。**限制要在使用者付出成本之前生效，不是之後。**
 *
 * 長邊 500px 是 `SCREENS §11` 定的：票根卡上的海報最寬是 56px、作品頁側欄
 * 220px，500px 已經涵蓋 2× DPR。
 *
 * ── 輸出一律 JPEG ────────────────────────────────────────────
 * bucket 只收 `image/jpeg` `image/png` `image/webp`。統一輸出 JPEG 的理由是
 * 大小可預期（PNG 的照片會比原檔更大），而海報是照片類內容。
 */
