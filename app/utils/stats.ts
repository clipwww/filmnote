...
export interface YearStats {
  ...
  /**
   * 月度趨勢的「歷年每月平均」虛線。⚠️ 分母是 **`years_observed`（曝光數）不是年份數**：
   * 實測 David 三月 13 次、一月 12 次、十月 12 次；用年份數 12 個月裡有 5 個會偏
   * （十月 2.00→1.85、一月 0.92→0.85、十二月 1.50→1.38）。不受 `p_year` 影響，前端別自己算。
   */
  monthly_baseline?: {
    month: number
...
/**
 * `{date, records, tickets}` → ECharts calendar 的 `[date, value]`。backend 刻意給具名欄位
 * （不把圖表函式庫的形狀綁進 API 契約），所以這一層是必要的、不是多餘的搬運。
 */
...
/**
 * 「當天看了兩場以上」的日子。要**另外疊一個 scatter 畫小點**，不能只靠色差：
 * `heat-4 ↔ heat-6` 只有亮 2.74:1／暗 2.53:1，低於 WCAG 1.4.11 的 3:1（§1.3）。
 * 而這些日子（雙片連映）正是整張圖唯一想讓人看見的東西。
 */
...
export interface HourGrid {
  ...
  missing: number
  /**
   * 每一欄（星期）的總和，長度固定 7。★★ **總和不進 `data`、不進 `max`。**
   * 實測 David 174 筆：單格最大 9、欄總和最大 60。進 `data` ⇒ max 變 60 ⇒ 真實的 112 格
   * 全塌進最淺兩階；沿用 max=9 ⇒ 7 個欄總和有 5 個被塗成最深階、與尖峰同色。兩種都零錯誤。
   */
  colTotals: number[]
...
/**
 * 攤成完整的 7 × N 格盤。① **全格都要餵值含 0**（§5.3-6）：heatmap 不畫空格（實測 3 筆的
 * 7×6 只產生 5 個 path），漏掉的會露出卡片底色像破洞；② `weekday` 是 isodow 不是 dow，
 * 當成 0-indexed 會整張平移一天**而且不報錯**；③ 00:00–02:00 折到最後標「午夜場」（§9.1）。
 */
...
/**
 * 同上，但**還沒到的月份給 null 不是 0**（`DESIGN_SYSTEM §5.3-8`）：看今年時 12 月的 0
 * 與 3 月的 0 意思完全不同，畫成 0 會讓折線在年中墜底、像「他七月就不看電影了」。
 * `year`／`today` 為 null 時退回 `monthlySeries()`——全期本來就沒有「未來的月份」。
 */
...
/*
 * ⚠️ 也不要在前端拿紀錄列表就地算：`/u/` 上別人拿得到的集合依 RLS 而異 ⇒ 同一個人的
 * 「歷年平均」會因為誰在看而不一樣。前端這側由 `verify-all` 的 frontend/monthly-baseline
 * 兩條守著（拿真實資料跟 DB 逐格對帳，並證明那組資料分辨得出兩種分母）。
 */
/**
 * 歷年每月平均（band 4 的虛線）。★ **只搬運不計算**，一律用 RPC 的 `avg_records`：
 * 分母是曝光數不是年份數，兩者對 David 在 12 個月裡有 5 個不同（十月 2.00/1.85、
 * 一月 0.92/0.85、二月 0.92/0.85、十一月 1.42/1.31、十二月 1.50/1.38）。拿不到回 null。
 */
...
/*
 * ⚠️ 這支**只管月份不管年份**：全期的「每個月」是季節性（十三年同月加總），指定年份時
 * 呼叫端必須另外加年份條件，否則切到 2019 點三月會列出十三年份的三月。
 * 放這裡不放頁面的 inline computed：vitest 摸不到 SFC（`SCREENS §9.2`）。
 */
/**
 * 某筆紀錄的 `watched_on` 是否落在某個月份（1..12）。⚠️ **一律字串切片不可 `new Date()`**：
 * `watched_on` 是台北牆上時間，`new Date('2026-03-01')` 當 UTC 午夜解析後 UTC 以西整個退一天
 * ⇒ 月初那筆掉到上個月而不報錯（圖上 12 筆、抽屜列 11 張，兩邊都「看起來正常」）。
 */
...
/**
 * 月度趨勢抽屜標題：全期「3 月＋全形空白＋12 場」、指定年份「2019 年 3 月＋全形空白＋4 場」。
 * ★ 全期刻意不寫「全部年度」（範圍由票根卡的 `show-year` 講）；★ 一定要帶場次——那是唯一能讓
 * 「圖上說 12、抽屜列 9」現形的地方（#169）。⚠️ 年月之間是半形空白，只有量詞前是 U+3000。
 */
...
export interface DistItem {
  ...
  records: number
  /**
   * 穩定識別（venue_id／format code／country 字串）。聚合列「其他 N 家」是 `null`。
   * ⚠️ **不可用 `name` 當識別**：影城同名分館不唯一；版本的「其他」是 `code='other'`，
   * 那是 RPC coalesce 出來的**真分類**（實測 David 5 筆）；國別的「未分類」是**空字串**。
   */
  key?: string | null
  /**
   * 只有「其他 N 家」那一列會有：被收進來的長尾原件，**順序與上游一致**（上游已排好）。
   * ⚠️ 不要在前端重排或改名——多一個會跟上游漂移而沒有人發現的地方。
...
/**
 * 長尾收成「其他 N 家」，被收進來的原件掛在那一列的 `rest` 上。保留原始筆數不轉百分比：
 * 長條長度由 `DistributionBars` 依第一列當基準算（這張圖是 HTML 排版不是 ECharts）。
 */
export function topWithRest(items: DistItem[], limit: number, restLabel: (n: number) => string): DistItem[] {
  // `<= limit` 放得下不用收；`=== limit + 1` 聚成「其他 1 家」會得到一顆點開只有一列的鈕。
  // ⚠️ 不是假想邊界：實測 David 全期的國別與版本**正好都是 5**（= 呼叫端的 limit），
  //    多一個沒看過的國別或版本就會踩到，距離只有一筆紀錄。
  if (items.length <= limit + 1)
    return [...items]
  // ⚠️ `slice()` 是淺拷貝，每一件原樣帶著自己的 `key`。不要重建物件——重建就會漏掉 `key`，
  //    症狀是「長條畫得出來、點下去抽屜永遠是空的」，typecheck 與既有測試都不會紅。
...
/**
 * ⚠️ **三種 kind 的預設值必須跟 RPC 的 group by 一模一樣**，否則圖上的數字與抽屜張數對不
 * 起來而兩邊都「看起來正常」：format `?? 'other'`（實測 David 真的有 5 筆走這條）、
 * country `?? ''`（未分類）、venue `?? ''`。放 utils 不放 SFC：vitest 摸不到（`SCREENS §9.2`）。
 */
...
/**
 * 分布長條的抽屜標題：`林口威秀影城` + 全形空白 + `120 場`。★ **一定要帶場次**（#169）——
 * 那是唯一能讓「長條說 120、抽屜列 96」被肉眼看見的地方，而且數字取自**長條上那個數字**
 * 不是抽屜實際列出幾張（同一個來源就永遠看不出不一致）。分隔符是 U+3000（`SCREENS §9.2`）。
 */
...
/**
 * 一個日期落在該年的第幾週（0 起算，週一為週首）。⚠️ 全程 `Date.UTC`：`new Date('2026-07-26')`
 * 當 UTC 午夜解析再轉本地，UTC 以西全退一天。不用 ISO 週號是因為 ISO 的第 1 週是「含 1/4 的
 * 那一週」，年初幾天會被算進前一年第 52/53 週，而年表是每年一列、跨年的格子沒地方放。
 */
...
/**
 * 全部年度的 `daily` → 年表的列。**收成「週」而不是「日」是資料逼出來的**：169 筆分散在
 * 12 年，日層級填滿率約 6%（7×53 有 96% 是空的，看起來是荒涼不是密度），收成週後跳到
...
/**
 * `YYYY-MM-DD` → isodow（1=週一 … 7=週日）。⚠️ 全程 UTC：`new Date(...).getDay()` 在 UTC 以西
 * 退一天而且不報錯——熱點圖的欄位與抽屜的內容會對不起來，但兩邊都「看起來正常」。
 */
...
/**
 * 某筆紀錄要不要算進「星期總和」那條抽屜。⚠️ **`!!watchedTime` 不能省，而且它在真實資料上
 * 永遠測不出來**：熱點圖的母體帶 `where watched_time is not null`，頁面手上的 records 是全部
 * ⇒ 少了它抽屜會比欄總和多；而 David 的 `records_without_time` 是 0，點一百次也不會現形。
 */
/*
 * 時段那條靠 `inHourRow(null, …) === false` 天然免疫，不需要這支。
 * 放 utils 不放 SFC：寫在 computed 裡 vitest 摸不到，上面那段就沒有東西守得住。
 */
...
/**
 * 多刷排行點一列時的抽屜標題：全期「片名＋U+3000＋10 次」、指定年份再多一段「2024 年」。
 * ★ **一定要帶次數**（#169）——唯一能讓「排行說 10、抽屜列 7」現形的地方，且數字取自排行
 * 那個數字不是實際列出幾張。★ 指定年份一定要帶年（多刷在全期與單年是兩件事，`SCREENS §9`）。
 */
/*
 * ⚠️ 這段刻意不貼字面的 U+3000：oxlint 的 `no-irregular-whitespace` 擋註解、不擋樣板字串
 * （`skipStrings` 預設開、`skipComments` 預設關）。實際分隔符看下面的樣板字串。
 */
 ...
 * 「少女與戰車最終章 第1話」與「《少女與戰車 最終章》 第3話」），slug 對未審核 UGC 會 404。
 * ⚠️ 年份用 `startsWith('2024-')` 不是 `getFullYear()`——同 `isoDow` 的時區理由。
 */
...
/**
 * ★ 樣本數不足時**不要下「最」的斷言**（`SCREENS §9c.3`）：「你最常在週六 10:00 進場，共 2 場」
 * ——2 場、樣本 8 筆，那是從雜訊長出來的斷言，比不給洞察更糟因為它看起來像一個發現。
 * **0 筆不畫圖、1–19 筆畫圖但不給斷言、20 筆以上才有洞察。**
 */
export const INSIGHT_MIN = 20

/**
 * ★★ **「最」要是真的**（2026-09-06 收緊 `SCREENS §9c.3`）。`INSIGHT_MIN` 守總筆數，守不到真正
 * 的問題：實測 David 174 場、67 個有值的格子，**最高 9、第二高也是 9、第三高 8**，舊邏輯照樣
 * 輸出「你最常在週六 10:00 進場，共 9 場」——那句話不是弱，是**不成立**（它在並列裡任意挑一個）。
 */
...
/**
 * 聚合型斷言（「X% 的場次在週末」）的門檻：至少要是平均分佈的這麼多倍。聚合橫跨很多格
 * 所以不受單格雜訊影響。基準線是「完全平均分佈時會是多少」：週末 3/7 = 42.9%，
 * 所以 1.4 倍 = 60%。實測 David 週末 **78.7%**（過關，而且是真的）。
 */
export const AGGREGATE_LIFT = 1.4

/**
 * ★ 而且它必須真的在講**多數**。只看「有沒有高過平均分佈」不夠：實測 David 的「週末晚上」是
 * **39%**、基準線 30%（3/7 × 8/16）、倍率 1.3 ⇒ 光看 lift 會放行，畫面上就會出現
 * 「你 39% 的場次在週五到週日的晚上」——技術上沒錯但讀起來是假的（61% 不是那樣）。
 */
export const AGGREGATE_MIN_SHARE = 50

/**
 * 這組數字裡的最大值有沒有「站出來」到可以被稱為「最」。
 * ⚠️ 兩頁共用這一支：各留一份判斷一定會漂移，而沒有人會把兩頁的同一句圖說擺在一起看。
 */
...
/**
 * 時段熱點圖的圖說。順序是**由穩健到脆弱**：先講跨很多格的聚合，站不出來的尖峰**不講**，
 * 什麼都站不出來時給一句純敘述（「沒有特別集中的一格」本身就是發現，而且是真的）。
 * `scopeLabel` 必要：每句圖說都要說得出涵蓋什麼範圍，否則兩種視角下同一句話數字差十倍。
 */
...
/**
 * 影城分布的圖說。同上兩頁共用，而且「主場」也要真的站得出來：實測 David 120/14/10…、
 * 佔 69% ⇒ 站得出來。若最高只比第二名多一點，「你的主場是 X」就是一句任意的話。
 */
...
/**
 * 金額的顯示字串。⚠️ **涵蓋不完整時要在數字上看得出來**，不能只靠底下一行小字——「以上」
 * 跟著數字走，使用者只截到一列時那個但書仍然在。看到部分資料卻不知道，比沒有這張圖更糟。
 */
/*
 * ⚠️ NT$0 不等於隱藏（`SCREENS §12.1`）：實測 David **2015 年 2 場、票價都記了、合計 NT$0**。
 * 第一版把「金額為 0」當成沒東西可看整列濾掉，2015 就從圖上消失（年表有、花費圖沒有）。
 * ⇒ 涵蓋完整時 0 是「免費」；不完整時仍是「NT$0 以上」——寫成免費就是把沒公開講成沒花錢。
 */
...
/**
 * 「每年花費」列尾的場次與張數。整列形狀是 `{金額} / {場數} 場 / {票數} 張`（2026-09-07 指定），
 * 金額那段由 `spendText()` 負責。⚠️ **分隔符是斜線不是站內慣例的全形空白**——刻意偏離
 * （`UserSpendSummary` 的「只涵蓋 3 / 5 筆紀錄」已有先例），不要順手改成全形空白或中點。
 */
/*
 * ⚠️ 場次與張數是兩個數字（實測 David 2019 年 25 場 37 張），順序不可對調；測資必須挑
 *   `records !== tickets` 的年份——用 2014（3 場 3 張）的話對調照樣綠（#175）。
 * ⚠️ 兩者都取自 by_year，與頁首同一個定義（實測加總正好等於 totals 的 174／250／57,873）。
 */
