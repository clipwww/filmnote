/**
 * 法律文件的 Markdown → 區塊樹（`SCREENS §15.1`）。
 *
 * ── 為什麼自己寫，而不是裝一個 markdown 套件 ─────────────────────────
 * 三份條款用到的語法是**封閉集合**：h1/h2/h3、段落、引言、有序與無序清單、
 * 表格、`**粗體**`、`` `行內碼` ``。沒有連結、沒有圖片、沒有程式碼區塊、
 * 沒有斜體。為這七種語法裝一個通用剖析器，換到的是一份要跟著升級的相依，
 * 以及一段**產生 HTML 字串**的路徑——而這一頁的內容雖然出自我們自己的資料庫，
 * 但把它接到 `v-html` 上就等於把「資料庫沒有被寫入 HTML」變成一條靠自律的保證。
 * 回傳結構化區塊、由 Vue 以文字節點算繪，那條保證由框架供應。
 *
 * ⚠️ 另一個理由更硬：`SCREENS §15.1` 明文禁止套用 Nuxt UI 的 prose 預設樣式
 * （字級與行高跟本站的紙不同調，且會把 `<code>` 渲染成等寬字，違反 DS §0）。
 * 也就是說**樣式一定要自己寫**，那麼剖析也自己寫並不多一件事。
 *
 * ── 換行接合：中文不能補空格 ─────────────────────────────────────
 * 原始檔在 80 欄手動斷行，Markdown 的語意是同一段。一般 markdown 實作以
 * **空格**接合軟換行——那對中文是錯的，「本服務不提供 電影片分級查詢」
 * 會多出一個看得見的空隙。這裡的規則是：接縫兩側只要有一邊是中日韓字元或
 * 全形標點就**直接相接**，兩邊都是拉丁才補空格。中英之間的視覺間距由
 * `text-autospace: normal` 供應（DS §2.5 第 4 條），不在 HTML 裡手打。
 */
...
/**
 * 條號 → 錨點 id。`## 3. 侵權通知` → `s3`、`### 3.1 聯繫窗口` → `s3-1`。
 *
 * ⚠️ **這個 id 是對外承諾的一部分**（`SCREENS §15.1`：侵權通知、客服回覆、
 * admin 備註都會指向特定一條）。從**條號**長出來而不是從標題文字長出來，
 * 是因為條號是條款自己的識別；標題文字改一個字，用文字做的錨點就死了，
 * 而寄出去的信裡那個 `#s4` 不會跟著改。
 */
...
export function parseLegalMarkdown(md: string): ParsedLegalDoc {
  ...
  while (i < lines.length) {
    ...
    if (heading) {
      ...
      // ⚠️ `display` 是**原文那一行**，不是拿 label 跟 text 重組出來的。
      // 重組會憑空長出標點：原文的 h2 是「3. 侵權通知」（有句點）、h3 是
      // 「3.1 聯繫窗口」（沒有），一律接 `. ` 會 render 成「1.1. 政府開放資料」。
      // 條號要怎麼寫是條款自己的事，顯示層不要替它決定。
    ...
    // ── 段落 ──────────────────────────────────────────────
    // ⚠️ 第一行**無條件**吃掉。上面每個分支都有「看起來像但不是」的漏網情形
    // （例如以 `|` 開頭卻沒有分隔列的行），若這裡照樣用 BLOCK_START_RE 擋，
    // 那一行會既不被任何分支消化、`i` 也不前進 ⇒ 整個剖析器停在無窮迴圈裡。
