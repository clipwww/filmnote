 ...
 * 播放版本的代碼 → 中文顯示名。
 *
 * `screening_format` 是查表不是 enum（SPEC：會持續長出成員的開放詞彙），
 * 所以顯示名必須從資料庫來，不能在前端寫死一份對照表——寫死的那份會在
 * 有人新增 `imax_laser` 之類的成員時默默過期，畫面直接印出原始代碼。
 *
 * 這張表對 anon 全開（RLS `using (true)`），內容與觀看者無關，
 * 所以 SSR 期間取它不會讓輸出因人而異。
 */
