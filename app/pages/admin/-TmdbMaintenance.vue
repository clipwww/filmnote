...
/**
 * TMDB 快取維護。⚠️ 檔名前綴的 `-` 不可省：`pages/` 掃 `.vue` **和** `.ts`，共用元件直接放
 * 進去會冒出一條 `/admin/-tmdb-maintenance` 路由（踩雷 #131）。顯式相對 import 不受影響。
 */
/*
 * 「Vercel 免費版不能用 cron」那個前提**不成立**（官方：cron 全方案都有，Hobby 每專案 100 條、
 * 最短一天一次、精度該小時內 ±59 分），`vercel.json` 的兩條排程照舊有效**沒有被取代**。
 * 這一頁要做的理由與 cron 無關：cron 唯一的觀測面是 `console.log`（站上看不出昨天有沒有跑）、
 * 失敗沒有重試入口、剛匯入要等到隔天 03:00、六個月條款的到期清除一天一次不夠即時。
 */
/*
 * ⇒ 這一頁的第一等公民是**上面那三個數字**不是兩顆按鈕：進來就看得到「到期待刷新幾列」。
 * ★ **按不完再按是設計不是缺陷**：`runTmdbRefresh()` 以時間預算收尾不以筆數收尾，超過預算
 *   就停止取新工作，剩下的列下次會再被 view 給出來 ⇒ `abortedBy === 'budget'` 是正常收尾，
 *   畫成錯誤的話管理者會以為壞了而不敢再按，正好把用法弄反。
 */
...
// ─────────────────────────────────────────────────────────────────────────────
/**
 * ★ 最大那一格（60 秒）就是**伺服器端的上限**（`MANUAL_MAX_BUDGET_MS`）。選單只給到上限，
 * 伺服器仍會再夾一次，而且把**實際生效的預算**放在回應裡——兩邊哪天分岔了畫面上看得出來。
 */
/*
 * 為什麼上限不是 300 秒（Vercel Hobby 的函式上限）：預算只決定「要不要再取新工作」，不會砍掉
 * 已經在飛的請求，最壞還要等 TMDB 的整套退避跑完（約 62 秒）；而真正的限制是**有個人在
 * 瀏覽器前面等**。
 */
...
// ─────────────────────────────────────────────────────────────────────────────
/**
 * ★ 四種收尾方式裡**只有一種是失敗**：`null` = 都處理完了；`'budget'` = 推進到預算上限
 * （**正常**，再按一次就好）；`'throttle'` = 被 TMDB 節流（要等，不要馬上再按）；
...
const outcome = computed(() => {
  ...
  if (r.abortedBy === 'fatal') {
    // ⚠️ 「這一輪沒有寫任何狀態」是**有條件的**，原本寫成無條件斷言是錯的（2026-09-08 抓到）：
    //    `runTmdbRefresh` 只跳過撞到 fatal 的那一列，在它之前完成的、以及其餘併發 worker 手上的
    //    列早就寫進 snapshot 並套用了。失效形狀：金鑰跑到一半被撤 ⇒ 紅字寫「沒有寫任何狀態」，
    //    正下方卻寫「刷新成功 12 / 套用回作品 12」，兩句話互相矛盾。
