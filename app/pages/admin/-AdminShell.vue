...
/**
 * `/admin/**` 三個佇列的外框（`SCREENS §14`）。三個長得一樣是刻意的：**David 是唯一的 staff
 * 而且不是每天做這件事** ⇒ 不做批次操作、統計儀表板、多人指派（那些是給團隊營運台用的），
 * 為的是「隔兩個月再打開」時不必重新學一次。**每個畫面只回答一個問題：這一筆怎麼處理。**
 */
/*
 * 版面是左列表、右詳情，760px 以下疊成一欄。⚠️ 左列表自己捲不讓它把頁面拉長：本專案已經有
 * 兩次「頁面攤開一萬七千／兩萬兩千 px」的紀錄，都是列表元件把所有列一次攤平在 document 流裡。
 */
/*
 * ⚠️ 沒有用 `UTable`：它的 `virtualize` 需要容器有確定高度（踩雷 #54），而這裡本來就是
 * 「一列一筆、點了看右邊」的清單不是表格。表格語彙留給 `/admin/films` 合併介面的並排比對。
 */
