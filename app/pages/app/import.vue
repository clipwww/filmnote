 ...
 */
/*
 * 四個版面決定都來自那批數字：① 影城排在片名前（只有 15 個要對、對完覆蓋全部 169 筆，而片名
 * 有 134 個——先做便宜的那段人才有動力做第二段）；② 依筆數由多到少（第一列就是 115 筆，
 * 對完少掉 68% 工作量）。
 */
/*
 * ③ **下拉一律不預選**：實測模糊比對會把「林口威秀」綁到「樹林秀泰影城」，而**錯配比不配更糟
 *    ——不配看得見、錯配看不見**；④ 把已對好的摺起來（首次命中 116/134 = 87%，平鋪要捲過
 *    116 列才找得到真正要處理的那 18 列）。
 */
/*
