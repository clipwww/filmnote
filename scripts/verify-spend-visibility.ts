 ...
 * `/u/` 上的金額，**三種觀看者各驗一組**。
 *
 *   pnpm tsx --env-file=.env scripts/verify-spend-visibility.ts
 *
 * ── 為什麼一定要真的打一次，而不是相信 RLS ────────────────────────────────
 * RLS 對，不代表**這一頁**對。金額要經過 RLS → `user_year_stats` 的聚合 →
 * `useUserSpend()` 的判斷 → 元件的 `v-if`。任何一層算錯，RLS 都還是綠的。
 * 而這一條的失敗方向是**把某個人的消費金額公開出去**，是全站最貴的那一種。
 *
 * ── 三種視角（`SCREENS §12`）─────────────────────────────────────────────
 * | 觀看者 | 應該看到 |
 * |---|---|
 * | 本人 | 全部票價 |
 * | 路人 ＋ `show_cost = true` | 公開紀錄的票價 |
 * | 路人 ＋ `show_cost = false` | **一列都讀不到 ⇒ 整個章節不存在** |
 *
 * ★ 「看不到」必須配一組「看得到」：只驗第三種會被「中介層把所有東西都拔光」
 *   矇混過去（§7 #102）。第二種就是那組對照——**同一個匿名 key、同一支查詢，
 *   只有 `show_cost` 不同**，所以兩者的差異只可能來自那個旗標本身。
 *
 * ── ⚠️ 這支會**暫時**把受測帳號的 `show_cost` 打開 ────────────────────────
 * 打開 → 讀 → **在 `finally` 裡關回去**，而且結束前會再查一次確認真的關回去了。
 * 這是 frontend 交接 §6.4 示範過的做法（「不需要第二個帳號」）。
 * ⚠️ 原值不是硬寫的 false，是**先讀出來再還原**——受測帳號本來就開著的話，
 *    關掉才是破壞。
 */
