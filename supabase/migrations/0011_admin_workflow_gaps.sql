...
--
-- ⚠️ **這一欄記的是「已通知」這件事，不是「系統發了信」。**
--    寄信服務 David 已裁定擱置，所以目前沒有任何自動化管道會通知使用者；
--    承辦人是用站外方式（例如手動寄信）聯絡之後，回來按這顆鈕留下時點。
--    這是刻意的分工：§90-4 第 4 款要舉證的是**通知這件事發生過**，
--    而不是「我們有一套寄信系統」。
--    ⇒ 這一欄**有寫入者**（就是這支 RPC），不是「存在但無意義」。
--    寄信服務上線後，那支服務應該呼叫**同一支 RPC**，而不是自己 update ——
--    否則會有兩個地方在寫同一個舉證欄位。
