...
| 250 | **★ ECharts 的 `axisLabel` 預設 `interval: 'auto'` 會在窄螢幕悄悄丟掉一半標籤——而標籤如果是點擊入口，那就是一半的入口不存在。** 實測 2026-09-20（正式站、375 寬）：`MonthlyTrend` 的 12 個月份標籤只畫得出 1／3／5／7／9／11，整條 309px 的軸每 4px 點一次，偶數月**在任何位置都點不到**；1280 下 12 個全在。⚠️ 最惡劣的是它**沒有任何錯誤、沒有 console 訊息、桌機驗收 100% 綠**，而該元件的註解正好寫著「月份標籤要可點，這不是加碼是必要條件」——理由是觸控裝置點不到 6px 的資料點。**為手機做的東西在手機上少了一半。** ⇒ 凡是把圖表庫算繪的文字當成互動入口，驗收必須在最窄的斷點做，而且要**逐一點過**而不是看設定。<br>**📌 2026-09-20 David 裁決：接受現況，不動 `interval`。** 另兩條路的代價更高（`interval: 0` 會讓中文標籤重疊；改印純數字會失去量詞）。⚠️ 這條踩雷因此**不是待修的 bug，是一個已知且被接受的落差**——**不要順手改成 `interval: 0`**。決定的落點在 `SCREENS §9c` 的裁決框。 | 2026-09-20 實測（`/u/clipwww`，Playwright + 系統 Chrome）|
| 251 | **「共用同一個 inflight promise」修不掉早退造成的順序反轉。** `ensureAllRecords()` 的 `if (loadedAll.value) return` 只花 1 個 microtask，而 `await inflight` 要 2 個 ⇒ 同一個 tick 內連點兩列時**後點的先寫、先點的後寫，最後停在先點的那一列**。四種情境實測對稱（A,B→A；B,A→B），兩個 handler 都確實被呼叫到。⚠️ 成因是紀錄數 ≤200 時那個 async IIFE **完全同步跑完**、在回傳前就把 `loadedAll` 設成 true；>200 筆的使用者反而是對的。**真人點不出來**（窗口只有一個 tick），所以這是不變量被違反而不是使用者症狀——但程式自己的註解就寫著「不可以早退」。⇒ 要修用序號守 `picked`，不要拿掉早退 | 2026-09-20 實測 |

| 252 | **★ 推薦 prompt 的可操作判準：ghost 提示 `esc` 與 `ctrl+u` 都清不掉。** 前三棒各攔下五則，判準一直是「輸入框裡出現你沒送過的字」——那需要協調者記得自己送過什麼。2026-09-20 實測到一個**不依賴記憶**的特徵：Claude Code 的推薦是 ghost 文字、不在輸入緩衝區裡，所以 `send-keys esc` 與 `ctrl+u` 都清不掉它；真的被打進去的字 `ctrl+u` 一定清得掉。⚠️ 這一輪三則的危險程度遞增：「照做只是浪費」→「Chrome 沒重啟卻說重啟好了，會對著沒有 CDP 的瀏覽器量」→「**數字我核過了，commit 吧**」（協調者當時根本還沒核）。★ 子 session 自己歸納出更準的判準：**「照做沒出事」不能當作分辨成功**——三則裡有兩則的內容碰巧與協調者真正的回覆同向，判準只能是「這串字我有沒有收到過」 | 2026-09-20 實測（三則，formats 與 dashboard 各自遇到）|
| 253 | **繞過 `scripts/db.ts` 直接用 raw pg 查，日期會被 JS `Date`→ISO 位移一天。** 實測：`2016-09-28` 印成 `2016-09-27T16:00:00.000Z`，差點被回報成「日期與協調者給的簡報對不上」。⚠️ **`scripts/db.ts:15-22` 的註解正是為了這件事而寫的**（node-postgres 預設把 date/timestamp 解析成 JS Date，`console.table` 再以 UTC 印出來；本專案刻意用 `date` + `time` 避開時區問題，斷言時被顯示層誤導會得出完全相反的結論），而繞過那支腳本就等於繞過那道保護。⇒ 查 DB 一律走 `pnpm db:sql`，不要自己開 pg client | 2026-09-20 實測（formats 的近失）|
| 254 | **★ 「故意把它弄壞一次」這個方法本身要先證明它真的弄壞了。** 本 repo 最有效的驗證手法是寫完斷言就故意破壞它守的東西、看是不是那一條變紅。2026-09-20 踩到它的盲點：macOS 的 **BSD `sed` 不支援 `\b`**，所以那次「弄壞」根本沒改到檔案，測試**照樣綠**——差一點被讀成「這條斷言是假的、可以刪」。⇒ 每一個弄壞法都要附一個「改完之後確實變成什麼樣」的數字（例如把替換的命中次數印出來），否則「弄壞了還是綠」有兩種解釋而你分不出是哪一種 | 2026-09-20 實測（dashboard）|

| 255 | **★ 部署進行中的 Vercel 專案會用「舊版」回應你的請求，而且回 200。** 實測 2026-09-20：`git push` 觸發部署後立刻打正式站驗收 `/u/clipwww?view=wall`，量到它與預設視角**幾乎一模一樣**（都 43 行海報）——照那份數字會得出「牆沒生效」的**錯誤結論**，實際上只是 production alias 還指著上一個部署。等 `vercel ls` 顯示 `Ready` 之後重量，同一條 URL 變成 **174 行海報／178KB vs 319KB**。⇒ 打正式站做任何驗收之前，**先確認那次部署的狀態是 Ready**（`vercel ls` 或 `vercel inspect`），不要只看 HTTP 狀態碼。這跟踩雷 #241（`127.0.0.1:3000` 回 200 但那是另一個專案的站）是同一個家族：**200 不代表你量到的是你以為的那個東西**，而「跟預期不符」與「量錯對象」在報告上長得一模一樣 | 2026-09-20 實測（`ac1e021` 那次 push）|
...
| 260 | **★★ 「只動註解嗎」這個問題，我連續用了四支壞掉的檢查器才答對。** 2026-09-20 註解清理時：① `grep -vE '^\s*(//|\*|<!--)'` 抓不到**多行 `<!-- -->` 的中間行** ⇒ 報 228 行假差異；② 同一支的迴圈版 ⇒ **差點拿去指控子 session 改了程式**；③ 改用正則 `/\*.*?\*/` 剝區塊註解 ⇒ **行註解裡的 `/api/**` 含有 `/*`**，吞掉後面的程式碼（子 session 抓到並主動回報）；④ 改用狀態機 ⇒ **正則字面值** `const re = /\*\*([^*]+)…/g` 的 `/\*` 同樣被當成區塊註解開頭，報 5 行假差異。⇒ 最後靠**三支互相獨立**的方法交叉才敢下結論：狀態機剝註解、`git diff` 增刪行是否全為註解、**`@vue/compiler-sfc` 把 template 編成 render function 再比**，再加 `pnpm build` 當最終仲裁。⚠️ 第三支有自己的陷阱：**`compileTemplate` 預設保留註解**，一定要 `compilerOptions: { comments: false }`，否則量到的差異就是註解本身。★ 真正的解法是**每一支檢查器都先餵已知答案做雙向自測**（未改動→綠、故意改一行→紅）——這是 #254「弄壞法本身要先證明它真的弄壞了」的鏡像：**檢查法本身要先證明它真的在檢查** | 2026-09-20 實測（四次，三次假紅燈一次被子 session 抓到）|
...
| 241 | **★ `localhost:3000` 與 `127.0.0.1:3000` 可能是兩個完全不同的 server，而錯的那個會回 200。** 交接筆記記的是「dev server 只聽 IPv6，所以 `curl 127.0.0.1:3000` 是 exit 7」——**那個描述已經不安全了**。2026-09-08 實測：`[::1]:3000` 是 filmnote 的 `nuxt dev`，而同時有另一個專案的 production build（`node .output/server/index.mjs`）佔著 `*:3000`（IPv4）。⇒ `curl 127.0.0.1:3000` 回 **200 加 1,909 bytes 的別人的網站**，不是連線失敗。exit 7 至少會逼你去看；200 只會讓你把別人的站當成自己的量。⚠️ 更陰險的是**開機順序**：dev server 還沒起來時 `localhost` 會退回 IPv4，所以「輪詢到 200 就算就緒」也是假的。⇒ **就緒判準與所有量測都要驗內容**（`document.title` 含「影記」、或 HTML 裡有已知字串），不要只看狀態碼。這跟 §7.6「判定必須看該不該，不是看有沒有」是同一條 | 2026-09-08 實測（瀏覽器驗收） |
...
> **📌 2026-09-20 David 的三則裁決（寫在這裡，因為它們各自關掉一條開著很久的線）**
>
> 1. **TMDB 直接匯入新片時，`review_state` 一律 `approved`，不進審核佇列。**
>    ⚠️ 代價要記清楚：片庫會出現**沒有在台灣上映過**的作品（TMDB 是全球片庫），
>    而政府分級資料是這個產品「片名以官方核准名稱為準」的權威來源。
>    ⇒ 實作時 `origin` 必須是 `'tmdb'`、`title_zh_source` 必須是 `'tmdb'`，
>    **並且要在 `seed_films()` 的 UPDATE 分支加一條**：政府資料日後帶著片名進來時，
>    把 `title_zh` 與 `title_zh_source` 一起翻成 `'gov'`。
>    不加那一條的話，`seed_films()` 第 613 行的 `case when title_zh_source = 'gov'`
>    會讓官方片名**永遠寫不進去**，而 `apply_tmdb_snapshot()` 每次快取更新還會把
>    TMDB 片名再寫一次 ⇒ 那列被鎖死在群眾翻譯的名字上，直接打穿第一條價值主張。
>    這條路本身在 schema 裡早就預留好了（`film_origin` 列舉有 `'tmdb'`、
>    `seed_films()` 已經用 `resolve_film('tmdb:' || tmdbId)` 去重）。
> 2. **Google 登入已於 2026-09-20 由 David 在正式站實測通過。**
>    跨兩輪的「登入從未驗過」到此結案。（協調者驗不了這一條：Supabase 的
>    redirect 白名單不在任何讀得到的端點裡，而 authorize 端點對**任何**
>    `redirect_to` 都放行——實測用 `.invalid` 網域當對照組確認過那個探針是瞎的。）
> 3. **下面第 34 條與「三振者能否用同一個 Google 帳號重新註冊」兩件，David 裁定擱置。**
>    （重新註冊已於 2026-09-25 裁決：可以，見下方 2026-09-25 第 3 則。）
>    ⚠️ 擱置**不是**已解決：兩件互相牽制（要擋三振者就得保留識別資料，
>    而那與「刪除我的所有資料」直接衝突），而且都有法律意涵。上線前要回來處理。
>
> **📌 2026-09-25 David 的裁決（接上面三則）**
>
> 1. **第 1 則補述**：那則要改的是 `seed_films()` 的**兩條**分支，原文只寫了 UPDATE 那一條。
>    ① INSERT 分支原本寫不出 `title_zh_source`（落回預設 `'gov'`）；② UPDATE 分支的 `case`
>    原本不看送進來的片名是誰的 ⇒ TMDB 片名會寫進 `gov` 的列。兩條都在
>    `0019_seed_films_title_zh_source.sql`（詳見 `docs/handoff/tmdb-import.md §9.1`）。
>    **放行套用 0019、放行真的匯入**（dry-run `--pages 3`：9/21 是 81 部、9/25 重跑是 83 部）。
> 2. **`title_zh_source='ugc'` 與 `'tmdb'` 同等對待**：政府片名進來時，值與來源一起翻成 `gov`。
>    已併進 0019（`42a99f7`）。`'admin'` 仍然永久不被政府資料更新。
> 3. **三振者可以重新註冊。** ⇒ 不需要為了擋三振者保留識別資料，上面第 3 則所說
>    「保留識別資料」與「刪除我的所有資料」的衝突，就重新註冊這一半而言不再存在。
>    **法遵證據保留期限仍然擱置**（第 34 條也仍擱置）。
> 4. `/app/records`：影城／版本選項**不限制**、搜尋**不比對日期**、換年份**不再清掉**
>    仍然存在的影城／版本（`3903949`）；篩選與頁碼**先不進 URL**。
> 5. **建一個非 staff 的測試帳號，常駐**（`#333`）。已建 `zz-nonstaff@example.com`，憑證在 `.env` 的
>    `TEST_NONSTAFF_*`；`record_update` 的 `film_usable_by` 已用它實測擋下已合併的片。
> 6. **後台要能匯入新作品**：`/admin` 的「從 TMDB 匯入新作品」（`806acee`）。起因是《奧德賽》（TMDB 1368337，
>    台灣 7/17 上映）已掉出 TMDB 的「上映中」，清單式匯入永遠抓不到它 ⇒ 加了片名搜尋與指定 id。
> 7. **「疑似已存在」要能直接補 TMDB id**（`d214e4f`／`5fa94a0`／`a21e948`）。補 id 不改政府片名；
>    `link_film_to_tmdb` 的靜默合併與 `film_ugc_review` 兩個坑由端點前置擋下（`tmdb-import.md §12`，#316／#317）。
