...
3. **一次匯入幾部。** `--pages 5` 的候選是 **116 部**（片庫 +4.2%），`--pages 3` 是 81 部。
   ...
   ⚠️ **使用者看得到的形狀**：新匯入的列快照是 `pending`，在下一次 `tmdb-refresh`
   cron 跑完之前，那 81–116 個 `/film/**` 頁面**沒有海報、沒有簡介、沒有片長也沒有年份**。
   ⇒ 「一次幾部」實際上是在決定「一次讓幾個空殼頁面上線多久」。
   ⚠️ `--limit N` 取的是 `fresh.slice(0, N)`，順序就是 TMDB 清單的順序
   （now_playing 各頁在前、upcoming 在後）。匯入腳本只印數字不印片名；
   要看**是哪 N 部**，跑 `pnpm tmdb:new-releases`，它會照同一個順序印出 ③ 的前 40 筆。
...
5. **`title_zh_source='tmdb'` 的列 + TMDB 片名進來**這一格（U4）目前維持現狀「不更新」。
   沒有裁決，但**實際代價是零、所以這比較像備註不是決定**：`apply_tmdb_snapshot()`
   的 `case when f.title_zh_source = 'tmdb' or f.title_zh = ''`（活體實查）本來就會在
