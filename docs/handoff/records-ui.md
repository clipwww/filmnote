...
### 11.2 ★ 第 1 條的瀏覽器驗收：全部過（原本記的「1 條沒過」已在 §11.3 撤回）
...
## 12. David 2026-09-25 的裁決（§9.4／§10.6 的卡點全部結掉）
...
| §10.6-5 非 staff 測試帳號（#333） | 建 | **未建**：要寫正式站的 `auth.users`，等權限放行 |
| §10.6-6 篩選／頁碼進 URL | 先不用 | 無 |

四關：`typecheck` ✅／`lint` ✅／`test` ✅ 547 條／`build` ✅（沒有 `externalized`）。
⚠️ **換年份的新行為沒有在瀏覽器裡點過**，只有純函式測試與靜態斷言。
