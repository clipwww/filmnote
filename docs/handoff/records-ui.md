...
### 11.3 ★★ 沒過的那一條：**開任何對話框都會把頁面捲回頂端，而且關掉不會還原**

實測（捲到 y=400，第 4 列的 `rect.top` = 48）：

| 動作 | scrollY | 第 4 列的 rect.top |
|---|---|---|
| 開抽屜前 | 400 | 48 |
| 開抽屜後 | **0** | **448** |
| 上一頁關掉後 | **0** | **448** |

**這不是量測假象**——`rect.top` 從 48 變成 448，畫面真的跳掉了 400px。

⚠️ **但這不是抽屜造成的，是這一頁三個對話框共有的既有行為。** 用兩個**完全不導航**的
對話框做對照，結果一模一樣：

- 備註全文 `UModal`：400 → 0（rect.top 48 → 448）
- 刪除確認 `UModal`：400 → 0（rect.top 48 → 448）

⇒ 它在備註對話框與刪除確認框存在的時候就已經在了，只是**從來沒有人拿「捲動位置沒變」
去驗過它**（`records.md §5` 那份「沒驗到的事」也沒收這一條）。

**病灶（查到的，不是猜的）**：開啟時只有**一次** scroll 事件、值是 0，而同一時刻
`document.activeElement` 移進了對話框內的一顆 `<button>`（`activeIsInDialog = true`）
⇒ 是 **focus 時沒有帶 `preventScroll: true`**，瀏覽器把焦點元素捲進視野。
排除掉的嫌疑：`body { overflow: hidden }` **不是**原因（手動設定並等 500ms，scrollY 穩定
停在 400、`maxScroll` 也沒變）；開著的時候手動捲回 400 是捲得動的（不是硬鎖）。

**為什麼我沒有自己修**：`USlideover` 的 emits 只有
`leave / after:leave / enter / after:enter / close:prevent / update:open`
——**沒有轉出 reka-ui 的 `openAutoFocus`**，所以擋不掉那次自動對焦。剩下的路是
在 `@after:enter` 把捲動位置捲回去（畫面會看得到 400→0→400 的閃動），或改動
共用的對焦行為（會同時影響備註與刪除兩個對話框，而且是 a11y 敏感的改動）。
⇒ **兩條都超出「六條」的範圍、而且會動到共用行為，列為卡點交給 David。**
...
7. **開對話框會把頁面捲回頂端、關掉不還原**（§11.3）。這是既有行為、三個對話框共有，
   但它讓第 1 條的「捲動位置沒變」**現在是不成立的**。兩條可能的路都會動到共用行為
   ⇒ 要不要修、修哪一條，David 決定。
