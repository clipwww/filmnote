...
### ① `isCacheable()` 是**推導**的，而 `swr` 在執行期讀不到

`server/middleware/strip-auth-on-cacheable.ts` 不再自己維護「哪些路由會被快取」
的清單，改成問 Nitro：
...
routeRules 只有一份，這支不再有機會與它不同步——日後有人給 `/legal/**` 加回
`isr`，這裡會**自動**開始拔 cookie，不必記得回來改。

⚠️ **判斷式裡沒有 `rules.swr`，那不是漏掉。** `NitroRouteRules` 的定義是
`Omit<NitroRouteConfig, 'redirect' | 'cors' | 'swr' | 'static'>`——`swr: true`
在**建置期**就被正規化成 `cache`，執行期讀 `rules.swr` 永遠是 `undefined`。
照 `nuxt.config.ts` 的字面去找 `swr` 會得到一個永遠不成立的判斷，
而症狀是「設了 swr 的路由不拔 cookie」、**畫面完全正常**。（§7 #117）

⚠️ 這支的失敗方向是**不安全的**（讀不到規則 ⇒ 判成不快取 ⇒ 不拔）。
所以真正守它的不是這支的邏輯，是 `verify:all` 裡那組直接對 HTML 的斷言。

### ② `notified_user_at` 記的是「通知發生過」，不是「系統發了信」

寄信服務 David 已裁定**擱置**。承辦人用站外方式（例如手動寄信）聯絡使用者之後，
回來呼叫 `admin_notify_user()` 留下時點。§90-4 第 4 款要舉證的正是**通知這件事
發生過**，不是「我們有一套寄信系統」——所以這是完整的舉證鏈，不是佔位欄位。

⚠️ **寄信服務上線後，那支服務必須呼叫同一支 RPC，不要自己 `update`。**
否則會有兩個地方在寫同一個舉證欄位，而那正是這種欄位最容易壞掉的方式：
兩邊對「什麼時候算通知過」的定義會漂移，而漂移之後沒有人分得出哪一個是對的。

（`deletion_requested_at` 則相反：它**沒有**寫入者，所以 0009 直接移除，
`verify-core` 的 E4 擋它長回來。兩者不是同一類，不要並列。）

### ③ 全期月度＝季節性，平均線的分母是**曝光數**

主 session 2026-09-06 裁決：全期的「每個月」是**季節性**（12 個月份桶、跨年度
加總），`/app` 與 `/u/` 一致。**不做 156 個月的時間序列**——走勢已經由年表
（band 1）在說，而同一個名字的圖在兩頁有兩種語意會讓使用者以為資料錯了。

`monthly_baseline` 是設計稿的「歷年每月平均」虛線，**不受 `p_year` 影響**
（來自未過濾的 `rec_all`）：指定年份時它是對照基準（「整年 N 場，比歷年平均的
X 場多／少」），全期時它與 `monthly` 同形狀而數值是平均。**一個欄位餵兩個用途，
不要做成兩套計算**——兩套一定會在某次修改後給出不一致的數字，而沒有人會發現，
因為沒有人會把兩頁的數字擺在一起看。

**分母**：從第一筆紀錄那個月到 `greatest(最後一筆, 今天)`，該月份實際經歷過幾次。
三個看起來都合理的錯誤選項見 §7 #123。實測 David：三月 13 次、一月 12 次
（2014-01 在起點之前）、十月 12 次（2026-10 還沒發生）。

⚠️ **這種錯誤守不住靠一致性斷言。** 驗 `avg = records / years_observed` 只驗到
內部一致——實測把分母一律改成 13，H1–H5 全綠而每個月的平均都偏。
守住它的是 **H6**：十二個月份的曝光加總必須等於觀測窗口的月份數（151），
分母寫成 13 會得到 156 而立刻紅。**那是一個與實作無關的不變量，不是實作的複本。**
