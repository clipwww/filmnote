/**
 * 片名正規化與版本標註處理。政府資料的中文片名常內嵌版本標註（實測 113 年 60/805 筆，
 * 7.5%），例如「紅豬(中文版)」。這些標註在 TMDB 上不存在，比對前必須剝除否則必然誤配。
 */

/**
 * 版本標註的樣式。修復類刻意寫成通用樣式而非窮舉——實際資料至少有「數位修復版」
 * 「4K修復版」「4K數位修復加長版」三種，每年都可能冒出新組合，窮舉必然漏掉。
 * ⚠️ 順序有意義：長的樣式必須排在短的之前，否則「4K數位修復加長版」會先被吃掉一部分。
 */
const VERSION_TERMS = [
  // (4K)(數位)修復(加長)(版) 的各種組合
  '\\d+K\\s*數位修復加長版',
...
/**
 * 自片名分離出版本標註。`title` 供顯示與後續比對，`note` 存入 certificate.versionNote；
 * 同一片名有多個標註時（罕見）以「、」串接。
 */
...
/**
 * 產生用於比對的正規化鍵。NFKC 會把全形英數與符號摺疊為半形（「：」→「:」），
 * 那是政府資料與 TMDB 之間最常見的差異來源之一。
 */
