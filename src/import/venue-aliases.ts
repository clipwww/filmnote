/**
 * 舊 log 的影廳口語簡稱 → 影視局影城名冊的統一編號。**這是人工對照表不是演算法**：
 * 15 個相異簡稱與 107 家官方名稱之間零個完全相同，而子字串比對會災難性地錯——
 * 「林口威秀」的子字串「威秀」會配到「台北京站威秀影城」，正解是「林口MITSUI OUTLET
 * PARK威秀影城」。簡稱只有 15 個，逐筆查證比任何模糊比對都便宜且正確。
 */
// 每一筆的 `reason` 寫的是判定依據不是描述：名冊更新時要能只讀這一欄就判斷該筆是否
// 仍然成立。查無對應者一律 `venueId: null` 並附 `question`，由人決定，不得先猜一個填著。
