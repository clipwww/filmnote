/**
 * 舊 log 片名 → TMDB id 的**人工**指定。
 *
 * 這張表存在的理由，以及它為什麼不是「把比對門檻調鬆一點」：
 *
 * 比對器（src/match/matcher.ts）最強的驗證訊號是片長交叉驗證，但舊 log
 * 沒有片長欄位，`runtimeOf` 只能回 null，那一關在這條路徑上形同關閉。
 * 護欄失效時放寬門檻，等於在最沒有把握的時候最敢猜——實測已經咬過一次：
 * 《Fate stay night Heaven's feel》靠前綴配到了系列**第二部**。
 * 所以呼叫端的策略是收緊（沒有精確訊號就降級為未命中），
 * 缺口用這張逐筆人工指定的表補，一部都不放給演算法猜。
 *
 * `reason` 不是裝飾：它是 Step 7 調整比對門檻時的依據——想知道
 * 「放寬哪一種樣式能多命中幾部、又會誤配幾部」，答案在這一欄的分佈裡。
 *
 * Step 7 的審核 UI 可直接把本表當「建議配對」的種子資料：
 * `logTitle` 是使用者輸入、`tmdbId` 是建議答案、`note` 是給審核者看的理由。
 */

/**
 * 比對器沒配到的原因分類。
 *
 * 皆為 169 筆舊 log 實測所見，不是預想出來的分類。
 */
