/**
 * 中文片名的人工對照表（編碼損毀修復）。**`reason` 欄是資產**：它記錄每一筆「為什麼
 * 需要人工介入」與「這個答案有多可靠」，日後重新檢視時那是唯一的依憑。
 * 進這張表的條件：中文片名編碼損毀**而且沒有可用的替代來源**（「有 TMDB 中文標題就用
 * 它」對沒配到 TMDB 的作品走不通，目前這兩筆正好都沒有 tmdb_id）。
 */
// 用 `permitNo` + `rocYear` 當鍵：不能用損毀的片名（那正是要修的東西，而且在不同階段
// 會被正規化成不同樣子），而 permitNo 跨年度不唯一（踩雷 #44）。
// ⚠️ 這張表只修**已知**的那幾筆；真正的守門員是 `defensive.ts` 的 `inspectTitleZh()`。
